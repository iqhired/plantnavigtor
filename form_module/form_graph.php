...
<body>
<!-- Main navbar -->
<?php $cust_cam_page_header = "Graphical representation of graph";
include("../header.php");
    ...
    <div class="content">
        ...
        <div class="panel panel-flat">
            <div class="panel-heading">
                ...
                <form action="" id="form_graph" class="form-horizontal" method="post" autocomplete="off">
                ...
                <table class="table datatable-basic">
                    ...
                    if ($button == "button1") {
                        if ($station != "" && $datefrom != "" && $dateto != "") {
                            $result = "SELECT * FROM `form_create` WHERE DATE_FORMAT(`created_by`,'%Y-%m-%d') >= '$datefrom' and DATE_FORMAT(`created_by`,'%Y-%m-%d') <= '$dateto' and station = '$station' " . $q_str . "ORDER BY form_create_id DESC";
                            $qur = mysqli_query($db,$result);
                        } else if ($station != "" && $user != "" && $datefrom == "" && $dateto == "") {
                            $qur = mysqli_query($db, "SELECT SELECT * FROM `form_create` WHERE  station = '$station' ");
                        } else if ($station != "" && $user == "" && $datefrom != "" && $dateto != "") {
                            $qur = mysqli_query($db, "SELECT * FROM `form_create` WHERE DATE_FORMAT(`created_by`,'%Y-%m-%d') >= '$datefrom' and DATE_FORMAT(`created_by`,'%Y-%m-%d') <= '$dateto' and station = '$station' " . $q_str . "ORDER BY form_user_data_id DESC ");
                        } else if ($station != "" && $user == "" && $datefrom == "" && $dateto == "") {
                            $qur = mysqli_query($db, "SELECT * FROM `form_create` WHERE station = '$station'");
                        } else if ($station == "" && $user != "" && $datefrom != "" && $dateto != "") {
                            $qur = mysqli_query($db, "SELECT * FROM `form_create` WHERE DATE_FORMAT(`created_by`,'%Y-%m-%d') >= '$datefrom' and DATE_FORMAT(`created_by`,'%Y-%m-%d') <= '$dateto' and station = '$station' " . $q_str . "ORDER BY form_user_data_id DESC");
                        } else if ($station == "" && $user != "" && $datefrom == "" && $dateto == "") {
                            $qur = mysqli_query($db, "SELECT * FROM `form_create` WHERE  station = '$station'");
                        } else if ($station == "" && $user == "" && $datefrom != "" && $dateto != "") {
                            $qur = mysqli_query($db, "SELECT * FROM `form_create` WHERE DATE_FORMAT(`created_by`,'%Y-%m-%d') >= '$datefrom' and DATE_FORMAT(`created_by`,'%Y-%m-%d') <= '$dateto' ");
                        }
                    ...
                    while ($rowc = mysqli_fetch_array($qur)) {
                        ...
                        $query1  = mysqli_query($db, "SELECT part_number, created_by , form_create_id FROM `form_create` where form_create_id = ' $form_id' LIMIT 1");
                        ...
                        <tr>
                            ...
                            <td><?php echo ++$counter; ?></td>
                                ...
                                <td class="tooltip">
                                    <a href="view_form_item_data.php?id=<?php echo $rowc['form_create_id']; ?>" class="btn btn-primary" style="background-color:#1e73be;"><i class="fa fa-eye" aria-hidden="true"></i></a>
